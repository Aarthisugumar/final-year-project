...
# Fixed enhanced LSTM model builder with matching shapes for Add()
def build_enhanced_lstm(input_shape):
    inputs = Input(shape=input_shape)

    x = Bidirectional(LSTM(64, return_sequences=True))(inputs)  # 128 features
    x = BatchNormalization()(x)
    x = Dropout(0.3)(x)

    x2 = Bidirectional(LSTM(64, return_sequences=True))(x)      # 128 features
    x2 = BatchNormalization()(x2)
    x2 = Dropout(0.3)(x2)

    x = Add()([x, x2])  # Shapes match now
